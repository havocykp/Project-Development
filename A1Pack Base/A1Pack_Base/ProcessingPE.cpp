...
{
	OutputDebugString(L"RVAToOffset Function Entry!");
...
{
	OutputDebugString(L"OffsetToRVA Function Entry!");
	PIMAGE_SECTION_HEADER pSectionHeader = IMAGE_FIRST_SECTION(m_pNt_Header);
...
{
	OutputDebugString(L"GetPeInfo Function Entry!");
	// 1. 判断映像指针是否有效
...
{
	OutputDebugString(L"FixReloc Function Entry!");
	// 1. 获取映像基址与代码段指针
	DWORD dwImageBase;
	PVOID lpCode;
...
{
	OutputDebugString(L"GetExpVarAddr Function Entry!");
...
{
	OutputDebugString(L"AddSection Function Entry!");
	PIMAGE_SECTION_HEADER pSectionHeader = IMAGE_FIRST_SECTION(m_pNt_Header);
...
{
	OutputDebugString(L"SetOEP Function Entry!");

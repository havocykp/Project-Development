...
CA1PackBaseDlg::CA1PackBaseDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_A1PACKBASE_DIALOG, pParent)
	, m_strPath(_T(""))
	, m_bShowMsg(FALSE)
{
...
BEGIN_MESSAGE_MAP(CA1PackBaseDlg, CDialogEx)
	...
	ON_BN_CLICKED(BTN_SELECT, &CA1PackBaseDlg::OnBnClickedSelect)
	ON_BN_CLICKED(BNT_START, &CA1PackBaseDlg::OnBnClickedStart)
...
void CA1PackBaseDlg::OnBnClickedSelect()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(true);
...
void CA1PackBaseDlg::OnDropFiles(HDROP hDropInfo)
{
	...
	if (_wcsicmp(szSuffix, L".exe") && _wcsicmp(szSuffix, L""))
	{
		MessageBox(L"你选择的不是EXE文件！");

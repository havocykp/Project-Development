...
IMPLEMENT_DYNAMIC(CExportDirList, CListCtrl)

CExportDirList::CExportDirList()
{
...
BEGIN_MESSAGE_MAP(CExportDirList, CListCtrl)
END_MESSAGE_MAP()

void CExportDirList::InsertColumnCustom(DWORD dwCount, ...)
{
	//设置扩展样式
...
void CExportDirList::InsertItemCustom(DWORD dwCount, ...)
{
	va_list list;
	va_start(list, dwCount);
	//首行位置插入一行
